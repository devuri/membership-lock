...
use MembershipLock\Settings\Register;
...
class BasicAuth
{
    ...
    protected $web_app;
    protected $option_name = 'mlock_basic_auth';
    ...
    {
        $this->username      = static::get_auth( 'username' );
        $this->user_password = static::get_auth( 'password' );
        $this->auth_option   = (bool) get_option( $this->option_name );
        $this->web_app       = get_option( 'blogname' );
    }

	protected static function get_auth( $auth )
	{
		if ( defined('BASIC_AUTH_USER') && 'username' === $auth ){
			return BASIC_AUTH_USER;
		} elseif ( defined('BASIC_AUTH_PASSWORD') && 'password' === $auth ){
     ...
     */
    protected function is_background_work()
    {
